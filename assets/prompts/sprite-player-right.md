...
- Asset: `assets/sprites/sprite-player-right.png`
- Kind: map sprite
...
- Adopted source generation: `/Users/goodapple/.codex/generated_images/019f49a3-db02-7a31-b08b-130b08b22272/exec-cf79fddc-7a43-4279-975f-4ce5ca627316.png`
...
`assets/sprites/sprite-player.png` の大きな青灰のフード、擦れた革鞄、霧へ溶ける外套裾、暗い旅装、2.5〜3頭身を保持。左向きの鏡像ではなく、フード・鞄・裾を右向き用に個別生成した。
...
Primary request: Create sprite-player-right, the unnamed traveler facing RIGHT/EAST in a true right-facing profile. Preserve the exact same character and outfit as Image 1: androgynous traveler, large weathered blue-grey hooded cloak, face mostly hidden by hood shadow, worn dark travel clothes, small scuffed brown leather shoulder bag, practical boots, cloak hem dissolving subtly into blue-grey mist.
Orientation definition: RIGHT means the face, hood opening, chest, knees, and toes point unmistakably toward the right edge of the canvas. Show a compact right-facing side silhouette; only a tiny shadowed facial profile may be visible. Do not face the viewer and do not turn into a 3/4 front pose. This must be independently drawn, not a simple mirror of the left version.
...
Identity and invariants: change only the direction. Keep the same hood size and shape, blue-grey cloak cut and mist-frayed hem, brown bag and straps, muted palette, broad painterly rendering, body scale, padding, and centered placement. Place the bag consistently on the same physical shoulder/hip as the reference, as visible from this right profile.
Map readability: bold simplified silhouette readable at 32px. The hood nose/opening, forward-pointing shoulder/chest, staggered tiny boots/toes, rear cloak trail, and satchel must make RIGHT immediately readable from outline alone. Strong value separation, no tiny details.
...
Constraints: follow docs/spec/world-lore.md 3.1 exactly. No invented weapons, armor, symbols, companions, monsters, extra bags, text, watermark, logo, frame, or UI. Not front-facing, not rear-facing, not left-facing, not 3/4, not isometric, not eye-level, not lying down, not realistic tall proportions.
```
...
- final alpha bbox: `(73, 21, 183, 231)`。四隅alpha: `[0, 0, 0, 0]`。
