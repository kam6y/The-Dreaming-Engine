# symbol-creaking-doll-right

- Asset: `assets/sprites/symbol-creaking-doll-right.png`
- Kind: map sprite
- Enemy: 軋み人形(きしみにんぎょう)
- Direction: right(右向き)
...
- Adopted source: `/Users/goodapple/.codex/generated_images/019f49df-0b0c-7d73-b326-88c7f5c32e5c/exec-2f451d56-0d04-48de-9f54-10e0ab786ef8.png`
...
大きな罅入り球形頭、継ぎ接ぎの布・木、真鍮の関節を固定。leftの鏡像は使わず、頭と肩の非対称を右向きにして個別生成した。
...
Asset id: symbol-creaking-doll-right.
...
Primary request: Render the exact same Creaking Doll as Image 1 facing RIGHT. Generate a fresh right-facing view independently; do not mirror or flip a left-facing output.
Camera: VERY HIGH TOP-DOWN-LEANING OVERWORLD VIEW matching Image 1. Show mostly the top crown of the huge round head, one right-facing edge of the cracked doll face, shoulders turned right, upper torso and joint blocks, with tiny compressed legs/feet below. The large head overlaps much of the torso; body is 2.5-to-3 heads tall at most. Absolutely not an eye-level side portrait or full-height character turnaround.
Direction: the doll's head and torso rotate toward the canvas right; the subtle nose/face plane points right, with the back of head toward the left. At 34px the asymmetry of head edge and shoulder turn must read right.
...
Map readability: bold large-head silhouette with a clear rightward front edge, short joint blocks, broad seams only.
...
Avoid: mirrored left output, eye-level side portrait, realistic tall anatomy, long limbs, living corpse, blood, gore, skulls, chains, armor, runes, extra objects, pixel art.
```

## 後処理・検収メモ

- 正面bbox長辺175pxに合わせて透過・中央配置し、256x256 RGBAへ正規化。leftとは別生成。
- final alpha bbox: `(88, 40, 167, 215)`。四隅alpha: `[0, 0, 0, 0]`。
- 緑優勢の可視画素0。34px縮小で頭・肩が右向き、脚が圧縮された俯瞰を確認。
