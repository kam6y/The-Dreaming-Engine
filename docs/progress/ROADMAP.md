...
## M26: AI応答キャッシュ・先行生成(BACKLOG「優先度: 低」4件目の展開。
## 防御検証済み結果のみのメモ化+夢シーンのコミット後オーバーラップ。防御仕様は追加方向のみ・一切弱めない)

- [x] M26-1: 仕様骨子の追記(BACKLOG展開に伴う骨子追記=CLAUDE.mdの例外に該当。
      骨子執筆=subagent、検収=オーケストレーター)。
      ai-integration.md「AI応答キャッシュ・先行生成(拡張: M26)」=採用案
      (A: 表示専用ターンのメモ化=状態変更effect 0件・出力壁通過・非フォールバックの
      検証済み成功ターンのみを記憶。適用2フロー=戦果描写(鍵=enemyId)+会話開始挨拶
      (鍵=NPC+好感度+話題+記憶要約+受注クエストの完全文脈フィンガープリント)/
      B: 夢シーンのコミット後オーバーラップ=日送り後の有料宿泊分岐でawaitを入眠演出と重ねる。
      空撃ちゼロ・同一1回・コスト中立)・対象外の明記(自由入力・クエスト生成・要約・
      投機的プリフェッチ)・ヒット時再検証省略の安全性根拠(決定論の純関数=同一鍵⇒同一判定)・
      レート保護整合(ヒットはセッション総数不消費・縮退中は不使用・許可判定は全て先に適用)・
      メモリのみ有界・モック/テスト方針・観測点(ai_cache_hit監査行)・不干渉条件
- [x] M26-2: 表示専用ターンのメモ化キャッシュ(**subagent担当**。UIは書かない):
      server純ロジック。AiTurnExecutor(flow-control/turn-executor.ts)のDreamMaster.run
      直前/直後にメモ化を挟む(ゲートキーパーの許可判定・直列化・クールダウン・縮退・
      セッション総数上限は現状のまま先に適用。縮退中はキャッシュ不使用)。
      記憶条件=骨子どおり(状態変更effect 0件・出力壁通過・非フォールバック/非却下の
      成功ターンのみ)。鍵=battleResult(enemyId)/会話開始挨拶(完全文脈フィンガープリント)。
      有界(フロー別LRU相当)・メモリのみ・非永続。監査ログai_cache_hit(機密マスク適用)。
      テスト用config/ai.jsonオーバーライドcache.enabled(既定true・本番デフォルト不変)。
      ユニットテスト=hit/miss・effect付き/フォールバック/却下/縮退ターン非記憶・
      ヒットでrun再呼び出しなし(モック呼び出し回数)・セッション総数不増加・
      鍵1要素変化でmiss。既存の攻撃テストA・モック呼び出し回数アサートを壊さない
      (防御アサートは緩めない)。既存E2E(スモーク)緑を確認
- [x] M26-3: 夢シーンの先行生成=オーバーラップ(server+client。**UI=オーケストレーター**):
      宿泊のWSメッセージ2フェーズ化(「眠りにつく」→演出中に夢を生成→「夢の顕現」)+
      クライアント入眠演出。**不変条件**=世界変化の適用順序(手順3夢→手順4適用→手順5セーブ)・
      <recent_play>/<world_state>のバイト一致・クールダウン/wasFree/縮退の内部判定・
      「AI失敗でもフォールバックで進行しセーブは必ず成立」。
      リスクが出ればcache-onlyへ縮退(プリフェッチ見送り・BACKLOG差し戻し)してよい。
      完了時にBACKLOG側へチェック+M26ゲート(test:e2e:full)

完了条件: 防御検証を通過した表示専用のAI応答(戦果描写・会話開始挨拶)が完全一致文脈の
再訪でAI呼び出しなしに即返され(セッション総数不消費・縮退中は不使用)、夢シーンのAI待ちが
入眠演出と重なって体感短縮される(呼び出し回数・コストは不変)。キャッシュはメモリのみ・
有界・非永続で、ai-guardrails.mdの全防御層・検証・出力壁・レート/コスト保護・ツール定義・
<world_state>はバイト不変(追加方向のみ)。既存ユニット・攻撃テストA・E2E
(スモーク+`test:e2e:full`)が緑のまま。`pnpm check`+`pnpm test:e2e`緑
(M26完了時は`pnpm test:e2e:full`も)。
