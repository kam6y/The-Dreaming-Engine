# ROADMAP — The Dreaming Engine

マイルストーンは上から順に進める。各マイルストーンの完了条件を満たし、
`pnpm check`(M1以降は`pnpm test:e2e`も)が緑で、コミット済みになったらチェックを付ける。
チェックの更新はチェック対象の作業をしたセッション自身が行う。

## M0: 環境構築

- [ ] pnpm workspaceモノレポ(`shared` / `client` / `server`)の雛形
- [ ] TypeScript strict / ESLint / Vitest / Playwright / `pnpm check` の整備
- [ ] `pnpm dev` でPhaserの空シーン(黒背景+タイトル文字)がブラウザに表示される
- [ ] `pnpm dev:mock` でサーバーとのWebSocket疎通(ping/pong)が確認できる
- [ ] `.env.example` / `.gitignore`(`.env` `saves/` `logs/` `node_modules/`)整備
- [ ] `README.md` にセットアップ手順(Auth token設定含む)を記載

完了条件: 上記すべて+E2Eで「タイトル文字が表示される」スモークが緑。

## M1: マップと移動

- [ ] グリッドベースのタイルマップ描画(プレースホルダータイル: 単色+簡易パターン)
- [ ] プレイヤーキャラの移動(WASD/矢印)・衝突判定・カメラ追従
- [ ] マップ間遷移(街⇔フィールド⇔ダンジョン各層)
- [ ] 街・フィールド・ダンジョン3層のマップデータ(JSON)作成
- [ ] NPC・調べられるオブジェクトの配置と、スペースキーでのインタラクション枠組み

完了条件: 街からダンジョン最下層まで歩いて到達できる。E2Eに移動スモーク追加。

## M2: ターン制戦闘

- [ ] 戦闘計算エンジン(純TSモジュール+ユニットテスト): ダメージ式・行動順・状態異常最小1種
- [ ] 戦闘シーンUI(コマンド: 攻撃/スキル/アイテム/逃走)
- [ ] シンボルエンカウント(フィールド・ダンジョンの敵シンボル接触で戦闘開始)
- [ ] 敵定義データ(雑魚3種+ボス1種)と敵行動パターン(決定論的)
- [ ] 経験値・レベルアップ・ゴールド・戦利品
- [ ] 全滅時の処理(街に戻る+ペナルティ)

完了条件: 雑魚戦とレベルアップが機能し、戦闘ユニットテストが揃う。E2Eに戦闘スモーク追加。

## M3: インベントリ・店・セーブ

- [ ] インベントリ(消耗品使用・所持上限)
- [ ] 商人NPCの店(購入・売却)
- [ ] 宿屋でのセーブ/ロード(サーバーが`saves/`にJSON保存、スロット1つで可)
- [ ] タイトル画面から「つづきから」でロード

完了条件: セーブ→タイトル→ロードで状態が完全復元される。E2Eにセーブ/ロードスモーク追加。

## M4: AI統合(DreamMaster)

- [ ] `docs/spec/ai-integration.md` に従いDreamMaster実装(Agent SDK + カスタムMCPツール6種)
- [ ] ツール検証層(純TSモジュール+ユニットテスト。仕様の上限・ホワイトリスト・クールダウンすべて)
- [ ] `MockDreamMaster`(定型応答)と `AI_MODE=mock|live` 切り替え
- [ ] NPC会話UI(自由入力+選択肢、ストリーミング表示、会話履歴・要約管理)
- [ ] 酒場でのサブクエスト動的生成(`propose_quest`)とクエストジャーナルUI
- [ ] 宿泊時の夢シーン→翌朝の世界変化(`trigger_world_event`)
- [ ] 戦闘勝利後の戦果描写(`narrate`)
- [ ] `docs/spec/ai-guardrails.md` の防御実装(入力壁・出力壁・監査ログ)と攻撃リグレッションテスト
- [ ] `pnpm test:ai-live`(実AI疎通の最小テスト)整備

完了条件: モックAIでE2E(会話・クエスト受注・夢シーン)が緑。攻撃テストが全て防がれる。

## M5: アセット生成と組み込み

- [ ] `docs/spec/asset-pipeline.md` のスタイルガイドに従い、Image Genで生成:
      タイトル画面 / 主人公+NPC4人の立ち絵 / 敵3種+ボスのグラフィック / 背景(街・フィールド・ダンジョン・戦闘) / UI装飾
- [ ] `assets/manifest.json` と `assets/prompts/` の整備
- [ ] ゲームへの組み込み(会話時立ち絵表示、戦闘背景+敵グラフィック、タイトル画面)
- [ ] 日本語フォント(ダークファンタジーに合うオープンライセンスのもの)の導入

完了条件: プレースホルダー画像が主要シーンから消え、見た目が「ゲーム」になっている。

## M6: 縦切り完成

- [ ] メインクエストライン実装(街到着→司祭の依頼→ダンジョン攻略→ボス「夢喰い」→エンディング)
- [ ] オープニング・エンディングの演出(静止画+テキストで可)
- [ ] BGM/SE(オープンライセンス素材 or 無音で可。JOURNALに判断を記録)
- [ ] バランス調整(Lv1開始→ボス撃破まで1-2時間、全滅が起こりうる難易度)
- [ ] 通しプレイのE2E(新規→メインクエスト完遂→エンディング到達をモックAIで自動化)
- [ ] README.md更新(遊び方・スクリーンショット)

完了条件: タイトル→エンディングまで人間が通しでプレイでき、`pnpm check`+全E2Eが緑。
**ここまでで縦切り完成。以降は `BACKLOG.md` に従い /loop で拡張する。**
