...
- [ ] M15-2: タイトル二重表示の解消+操作キーの説明画面(タイトルまたはメニューから
      参照可能なヘルプ。UI=オーケストレーター自身が実装)。
      注意: DOMの h1/#connection-status はE2Eが参照するため削除せず
      視覚的に隠す(sr-only等)。あわせて各オーバーレイの操作ヒント表記を統一する
- [ ] M15-3: UX点検のP2群+軽量P3の修正(UI=オーケストレーター、ロジック=subagent):
      設定音量の左右キー増減+ヒント表示 / 消耗品の効果説明1行(もちもの・店) /
      対話可能オブジェクト(看板・宝箱・採取)の視覚的手掛かり /
      戦闘勝利時の獲得EXP・ゴールド表示 / 接続ステータスの接続時自動非表示 /
      セーブ有時のタイトル初期カーソル=つづきから / 宿確認文の閉じ括弧折返し調整
- [ ] M15-4: UX点検の残りP3の修正(裁量で一部見送り可。見送りはJOURNALに理由を記録):
