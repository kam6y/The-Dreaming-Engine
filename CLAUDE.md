# CLAUDE.md — The Dreaming Engine 開発憲法

このリポジトリは、ゲーム実行中に生成AI(Claude Agent SDK)を組み込んだ日本語ダークファンタジー2D RPG
「The Dreaming Engine」を、Claude Codeの長時間自律駆動で開発するプロジェクトである。
あなた(Claude Code)はこのファイルの規約に**常に**従うこと。
...
| `docs/spec/asset-pipeline.md` | 画像アセット生成(codex委譲)の規約 |
...
- 画像アセットはClaude Codeが自作しない(SVG手描き・AAアート等での代用も不可。
  コード生成のプレースホルダー(単色+簡易パターン)のみ例外)
- 新しい画像が必要な場合は `docs/spec/asset-pipeline.md` に従い、
  codex(`mcp__codex__codex`)へ委譲して生成する。委譲時はスタイルガイド・
  納品先(`assets/`)・`world-lore.md`の外見典拠を必ず依頼に含める
- codexの納品物はClaude Codeが検収する(manifest整合・パス・命名・サイズの確認)。
  コミットもClaude Code側で行う
- codex MCPが使えない・生成が失敗し続ける場合は、JOURNALに「人間確認待ち」として
  記録し、プレースホルダーのまま先へ進む
...
- `main` ブランチへの直接コミット・push禁止。作業は `claude/dev` で行う
